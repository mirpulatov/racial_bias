...
class Resnet50_256(nn.Module):
    ...
    def forward(self, data, calc_scores=True):
        ...
        ########################### Changes #################################### 
        #classifier_8631_f_preflatten = self.classifier_8631_f(feat_extract)
        #classifier_8631_f = classifier_8631_f_preflatten.view(classifier_8631_f_preflatten.size(0), -1)
        
        embeddings = feat_extract.view(feat_extract.size(0), -1)
        adaptation_layers_output = [embeddings]
        
        classifier = None

        if calc_scores:
          classifier_8631_f_preflatten = self.classifier_8631_f(feat_extract)
          classifier = classifier_8631_f_preflatten.view(classifier_8631_f_preflatten.size(0), -1)
        
        return embeddings, adaptation_layers_output, classifier
